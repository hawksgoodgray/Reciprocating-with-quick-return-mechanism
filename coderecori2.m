...
% graphic objects (no black dash baseline)พพพ
...
%%h_slider  = plot(NaN,NaN,'s','MarkerFaceColor',[1 0.5 0]);% rack marker (center)
...
for k = 1:N
    ...
    if abs(sin(phi)) < 1e-6
        % if almost horizontal, keep previous contact if possible
        H = [B(1), rack_y];
    else
